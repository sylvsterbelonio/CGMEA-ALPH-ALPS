...
Public Class frmLGUFinder
    Inherits System.Windows.Forms.Form

#Region "frmLGUFinder Variable Declaration"

    Private colLGU As New Collection
    Private colButton As New Collection
    Private tooltipLGU As New ToolTip
...
#Region "frmLGUFinder Public Property Variable Declaration"
...
#Region "frmLGUFinder Form Sub"

    Private Sub frmLGUFinder_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Cursor = WaitCursor
        Initialize_All()
        Me.Cursor = Arrow
    End Sub

    Private Sub frmLGUFinder_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
        If Use_Record_State = USE_STATE Then
            btnEdit.PerformClick()
        End If
    End Sub

    Private Sub frmLGUFinder_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress, txtLGUCode.KeyPress, txtLGUName.KeyPress ', udgFinder.KeyPress
...
#Region "frmLGUFinder User Defined Private Sub"
...
Add_Rec:
                ...
                Case "&view", "&edit"
                    ...
                    clsLGU.Init_Flag = intLGUId
                    If clsLGU.Selected_Record() Then
                        Dim NwfrmLGU As frmLGU
                        frmTitle = "Local Government Unit - " & clsLGU.LGU_Name
...
Use_Rec:
                    With frmMainUser
                        RaiseEvent Use_Clicked(intLGUId, LGUName, LGUCode)
                        Use_Record_State = ""
                        Me.Close()
                    End With
                Case "&cancel"
    ...
    Private Sub Search_Crit()
        ...
        With clsLGU
            ...
            .CritLGU_Code = Trim(Me.txtLGUCode.Text)
            ...
            If dtable.Rows.Count > 0 Then
                ...
                If Use_Record_State = USE_STATE Then
                    If dgvFinder.SelectedRows.Count > 0 Then
                        intLGUId = dgvFinder.SelectedRows(0).Cells(0).Value 'udgFinder.Selected.Rows(0).Cells(0).Value
                        LGUName = dgvFinder.SelectedRows(0).Cells(2).Value 'udgFinder.Selected.Rows(0).Cells(1).Value
                        LGUCode = dgvFinder.SelectedRows(0).Cells(1).Value 'udgFinder.Selected.Rows(0).Cells(4).Value
                    End If
                End If
            Else
    ...
    Private Sub Initialize_All()
        ...
        Me.tooltipLGU = clsCommon.SetUp_ToolTip_Finder(colButton)
        Create_Context_Menu()
    End Sub

    Private Sub Event_Handler(ByVal strMessage As String, ByVal blnSuccessfl As Boolean) Handles clsLGU.MsgArrival
        Me.Cursor = Arrow
...
#Region "frmLGUFinder DataGridView Private Sub"
    ...
    Private Sub dgvFinder_Initialize()
        With dgvFinder
            ...
            Dim dgvColumnLGUCode = New DataGridViewTextBoxColumn
            ...
            With dgvColumnLGUCode
                .Visible = True
                .DataPropertyName = "LGUCode"
                .ToolTipText = "LGU Code"
                .HeaderText = "LGU CODE"
                .Width = 200
            End With
...
#Region "frmLGUFinder Protected Overrides Sub"
