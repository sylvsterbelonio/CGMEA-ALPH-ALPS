...
Public Class frmSystemAccessLevelFinder
    Inherits System.Windows.Forms.Form

#Region "frmSystemAccessLevelFinder Variable Declaration"

    Private colSystemAccessLevel As New Collection
    Private colButton As New Collection
    Private tooltipSystemAccessLevel As New ToolTip
...
#Region "frmSystemAccessLevelFinder Form Sub"

    Private Sub frmSystemAccessLevelFinder_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Cursor = WaitCursor
        Initialize_All()
        Me.Cursor = Arrow
    End Sub

    Private Sub frmSystemAccessLevelFinder_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
...
#Region "frmSystemAccessLevelFinder User Defined Private Sub"
    ...
    Private Sub txtRoleName_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtRoleName.KeyPress, txtRoleDescription.KeyPress
...
Add_Rec:
                ...
                Case "&view", "&edit"
                    ...
                    clsSystemAccessLevel.Init_Flag = intRoleID
                    If clsSystemAccessLevel.Selected_Record() Then
                        Dim NwfrmSystemAccessLevel As frmSystemAccessLevel
                        frmTitle = "System Access Level - " & clsSystemAccessLevel.Role_Name
                        ...
                        Else
                            ...
                            With NwfrmSystemAccessLevel
                                ...
                                If Not LCase(btnObject.Name) = "btnview" Then
                                    If intRoleID = 0 Then
                                        clsCommon.Prompt_User("information", "System access level can not be modified for built-in system administrator.")
                                        Exit Sub
                                    End If
                                    .State = EDIT_STATE
                                    .Set_Form_State()
...
Use_Rec:
                    With frmMainUser
                        RaiseEvent Use_Clicked(intRoleID, RoleName)
                        Use_Record_State = ""
                        Me.Close()
                    End With
                Case "&cancel"
    ...
    Private Sub Search_Crit()
        ...
        With clsSystemAccessLevel
            ...
            dtable = New DataTable
            .Init_Flag = 1
            .Crit_RoleName = Trim(Me.txtRoleName.Text)
            .Crit_RoleDesc = Trim(Me.txtRoleDescription.Text)
            ...
            If dtable.Rows.Count > 0 Then
                ...
                If Use_Record_State = USE_STATE Then
                    If dgvFinder.SelectedRows.Count > 0 Then
                        intRoleID = dgvFinder.SelectedRows(0).Cells(0).Value
                        RoleName = dgvFinder.SelectedRows(0).Cells(1).Value
                    End If
                End If
            Else
    ...
    Private Sub Clear_All_Controls()
        colSystemAccessLevel = New Collection
    ...
    Private Sub Initialize_All()
        ...
        Me.tooltipSystemAccessLevel = clsCommon.SetUp_ToolTip_Finder(colButton)
        Create_Context_Menu()
    End Sub
...
#Region "frmSystemAccessLevelFinder DataGridView Private Sub"
    ...
    Private Sub dgvFinder_Initialize()
        With dgvFinder
            ...
            Dim dgvColumnRoleDescription = New DataGridViewTextBoxColumn
            ...
            With dgvColumnRoleDescription
                .Visible = True
                .DataPropertyName = "roleDescription"
            ...
            .Columns.Add(dgvColumnRoleDescription)
...
#Region "frmSystemAccessLevelFinder Protected Overrides Sub"
