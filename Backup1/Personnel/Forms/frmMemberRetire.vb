...
Public Class frmMemberRetire
    Inherits System.Windows.Forms.Form

    Private WithEvents clsCommon As New Common.Common
    Private WithEvents clsPermission As New DataAccess.Permission(gConnStringFileName)
    Private WithEvents clsMemberRetire As New MemberManagement.MemberManagement
    ...
    Private WithEvents frmFinder As frmMemberRetireFinder
    ...
    Private ColMemberRetire As New Collection
...
#Region "frmMemberRetire Main Form Private Sub"

    Private Sub frmMemberRetire_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        clsCommon.Set_Toolbar_Tooltips(Me.tbrMainForm)
        Me.Cursor = WaitCursor
        Initialize_Form()
        If State = EDIT_STATE Or State = VIEW_STATE Then
            clsMemberRetire.Init_Flag = RecordId
            clsMemberRetire.Selected_Record()
            Assign_Data()
        End If
        Me.Cursor = Arrow
    End Sub

    Private Sub frmMemberRetire_Closing(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles MyBase.Closing
    ...
    Private Sub frmMemberRetire_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Disposed
        Disposed_Class()
    End Sub

    Private Sub frmMemberRetire_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress, txtMemberId.KeyPress, txtMemberName.KeyPress, txtRemarks.KeyPress, dtpManagementDt.KeyPress, cboRetireFl.KeyPress
...
#Region "frmMemberRetire ToolBar Private Sub"
    ...
    Private Sub tbrMainForm_ButtonClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ToolBarButtonClickEventArgs) Handles tbrMainForm.ButtonClick
        If TypeOf (sender) Is ToolBar Then
            ...
            Select Case e.Button.ImageIndex()
                Case 0 'add
add_rec:
                    Dim NwfrmMemberRetire As New frmMemberRetire
                    frmTitle = "Member Retire"
                    If blnUseFinder Then
                        State = ADD_STATE
                        clsCommon.ClearControls(ColMemberRetire)
                        Set_Form_State()
                        RecordId = 0
                    ...
                    Else
                        If clsCommon.CheckIfRecordIsLoaded(Me.ParentForm, frmTitle) Then
                            NwfrmMemberRetire = clsCommon.GetRecordChildForm(Me.ParentForm, frmTitle)
                            NwfrmMemberRetire.Activate()
                        Else
                            State = ADD_STATE
                            clsCommon.ClearControls(ColMemberRetire)
                            Set_Form_State()
...
find_rec:
                    If Not blnUseFinder Then
                        Dim frmFinder As frmMemberRetireFinder
                        frmTitle = "Member Retire Finder"
                        If clsCommon.CheckIfRecordIsLoaded(Me.ParentForm, frmTitle) Then
                        ...
                        Else
                            frmFinder = New frmMemberRetireFinder
                            With frmFinder
                                .Management_Id = 0
...
save_rec:
                    If clsCommon.Required_Validation_List(ColRequired) Then
                        ...
                        With clsMemberRetire
                            ...
                            .Retire_Fl = Trim(cboRetireFl.Text)
                            ...
                            If .Save_Record() Then
                                ...
                                clsMember.Init_Flag = clsMemberRetire.Member_Id
                                clsMember.Selected_Record()
                                Me.Text = "Member Retire - " & clsMember.Last_Name & IIf(Len(clsMember._suffixName) > 0, " " & clsMember._suffixName, "") & ", " & clsMember.First_Name & IIf(Len(clsMember.Middle_Name) > 0, " " & clsMember.Middle_Name, "")
...
#Region "frmMemberRetire User Defined Private Sub"
    ...
    Private Sub Assign_Data()
        ...
        'clsDepartment.Init_Flag = clsMemberRetire.ToDepartment_Id
        'clsDepartment.Selected_Record()
        'txtDepartmentName.Text = clsDepartment.Department_Name

        'If clsMemberRetire.ForMember_Id > 0 Then
        '    clsMember.Init_Flag = clsMemberRetire.ForMember_Id
        '    clsMember.Selected_Record()
    ...
    Private Sub Disposed_Class()
        clsMemberRetire = Nothing
        clsCommon = Nothing
        ColDisplay = Nothing
        ColRequired = Nothing
        ColMemberRetire = Nothing
    End Sub
    ...
    Public Sub Set_Form_State()
        ...
        If State = "View" Then
            lblRetirementType.Text = "Retirement Type"
    ...
    Private Sub Event_Handler(ByVal strMessage As String, ByVal blnSuccessFl As Boolean) Handles clsMemberRetire.MsgArrival, clsCommon.MsgArrival
        Me.Cursor = Arrow
    ...
    Private Sub Set_Required_Tags()
        ...
        cboRetireFl.Tag = "Retirement Type"
